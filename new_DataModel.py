...
class DataHandler(object):
  ...
  def __init__(self, df=pd.DataFrame(), rec_data=0):
    if not df.empty:
      ...
      self.raw_txns_path = 'chipotme/data/default.csv'
      self.raw_df = pd.read_csv(self.raw_txns_path)
    self.store_records_path = 'chipotme/data/chipotle_store_info.csv'
  ...
  def log_transactions(self, df):
    ...
    file_path = 'chipotme/data/anon/%s_chipotle_final.csv'%(str(file_unique))
    df.to_csv(file_path)
  ...
  def enhance_with_StoreInfo(self):
    ...
    for x in self.pdf.store_no:
      ...
      if x not in list(srdf.store_no.apply(int))+already_checked:
        ...
        continue
    srdf.drop_duplicates().to_csv('chipotme/data/chipotle_store_info.csv', index=False)
    return srdf
