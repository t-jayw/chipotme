...
class MapBox(object):
  ...
  def return_normalized(self, column):
    print("YO")
    scale = 15
    col = column
    col_min = col.min()
  ...
  def create_piv_for_geo(self):
    ...
    df_geo['size'] = self.return_normalized(amounts['sum']) + \
                      self.return_normalized(amounts['count'])
    df_geo['norm_size'] = self.return_normalized(df_geo['size']) 
    return df_geo
